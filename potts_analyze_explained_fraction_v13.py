...
Analyzes Potts descriptors for injectivity:
  ...
  3) (Optional) Global PCA compression of outcome space Y using streaming Oja updates
     - selects m = min #components capturing >= 95% energy
...
def _get_corr2d_shape(desc_h5: Path) -> Tuple[int, int, int]:
    with h5py.File(str(desc_h5), "r") as f:
        mean2d_ds = f["correlations/correlations_2d_mean"]
        shape = mean2d_ds.shape
        # expected: (N, P, H, W)
        if len(shape) != 4:
            raise ValueError(f"Unexpected corr2d shape: {shape}")
        _, P, H, W = shape
        return int(P), int(H), int(W)


def plot_pca_top_components_corr2d(
    Q: torch.Tensor,
    desc_h5: Path,
    cfg: Config,
    outbase: Path,
    ...
    if meta["descriptor"] != "corr2d":
        return

    P, H, W = _get_corr2d_shape(desc_h5)
    ph_dim = int(meta["ph_dim"])
    y_dim = int(meta["feat_dim"] + (ph_dim if cfg.prepend_phase_fractions_to_Y else 0))

    Qcpu = Q.detach().to("cpu", dtype=torch.float32)
    k = min(int(n_components), int(Qcpu.shape[1]))
    if k <= 0:
        return

    # Split into phase fractions and corr2d feature part
    ...
    fig, axes = plt.subplots(k, P, figsize=(2.2 * P, 2.2 * k), dpi=dpi)
    if k == 1:
...
def heatmap_explained_frac_with_pca3d(
    temp: np.ndarray,
    frac: np.ndarray,
    Z: np.ndarray,
    Yp: np.ndarray,
    ...
    - Left: 2D heatmap of explained fraction
    - Right: 3D scatter plot of first 3 PCA components colored by Z
    """
    temp = temp.astype(np.float64)
    frac = frac.astype(np.float64)
    Z = Z.astype(np.float64)
    ...
    # Create figure with GridSpec for left (heatmap) and right (3D)
    ...
    ax_hm.set_ylabel(ylabel, fontsize=11)
    if overlay_lines is not None:
        for xline, yline, kw in overlay_lines:
    ...
    if Yp.shape[1] >= 3:
        ...
        pc1 = Yp[:, 0]
        pc2 = Yp[:, 1]
        pc3 = np.zeros(Yp.shape[0])
    else:
        pc1 = Yp[:, 0]
        pc2 = np.zeros(Yp.shape[0])
        pc3 = np.zeros(Yp.shape[0])
...
def heatmap_explained_frac_with_pca3d_azimuth_series(
    temp: np.ndarray,
    frac: np.ndarray,
    Z: np.ndarray,
    Yp: np.ndarray,
    ...
    ylabel: str = "fraction_initial",
    overlay_lines: Optional[List[Tuple[np.ndarray, np.ndarray, Dict[str, Any]]]] = None,
    azimuth_angles: Optional[List[int]] = None,
    dpi: int = 250,
) -> None:
    """
    ...
    azimuth_angles : list of int
        Azimuth angles (in degrees) to generate. Default: [0, 60, 120, 180, 240, 300]
    """
    ...
    # Generate plot for each azimuth angle
    for azim in azimuth_angles:
        ...
        ax_3d.set_ylabel("PC2", fontsize=10, labelpad=8)
        ax_3d.set_zlabel("PC3", fontsize=10, labelpad=8)
        ax_3d.set_title(f"3D PCA Projection", fontsize=12, fontweight="bold")
        ...
        fig.savefig(str(outpath) + ".png", bbox_inches="tight", dpi=dpi)
        fig.savefig(str(outpath) + ".pdf", bbox_inches="tight")
        plt.close(fig)
...
def main() -> None:
    ...
    if cfg.use_global_pca:
        ...
        # Save top PCA components as corr2d images (if applicable)
        plot_pca_top_components_corr2d(
            Q=Q,
            desc_h5=desc_h5,
            cfg=cfg,
            outbase=figs_dir / "pca_top4_components_corr2d",
            n_components=4,
            dpi=cfg.dpi,
        )

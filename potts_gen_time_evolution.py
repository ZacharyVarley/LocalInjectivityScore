...
# -*- coding: utf-8 -*-
"""
potts_gen_time_evolution.py

Potts simulator that saves RAW final spin configurations at multiple time snapshots
without restarting the simulation. Snapshots are taken at steps 100, 200, 300 by
default, and all data are stored under a non-timestamped folder for privacy.

Outputs (new IO style):
  potts_data_time_evolution/time_evolution/
    potts_sims_q{q}_{H}x{W}_steps{max_steps}.h5
    potts_sims_q{q}_{H}x{W}_steps{max_steps}.json

H5 layout:
  parameters/temperature            (N,)
  parameters/fraction_initial       (N,)
  states/final_spins_step100        (N, R, H, W) int8
  states/final_spins_step200        (N, R, H, W) int8
  states/final_spins_step300        (N, R, H, W) int8
...
Notes:
  ...
  - For 200 and 300, the simulation continues from the 100 snapshot.
"""
...
class Config:
    ...
    # Snapshot steps are cumulative; simulation continues between them.
    snapshot_steps: Tuple[int, int, int] = (100, 200, 300)
    ...
    out_root: str = "potts_data_time_evolution"
...
def create_initial_states(
    ...
    for i, (fraction, seed) in enumerate(zip(fractions, seeds)):
        _ = seed  # deterministic seed path preserved for future use
...
def _parse_snapshot_steps(steps_str: str) -> List[int]:
    parts = [p.strip() for p in steps_str.split(",") if p.strip()]
    steps = sorted({int(p) for p in parts})
    if not steps:
        raise ValueError("snapshot_steps must include at least one positive integer")
    if min(steps) <= 0:
        raise ValueError("snapshot_steps must be positive integers")
    return steps
...
def main() -> None:
    ...
    ap.add_argument("--snapshot_steps", type=str, default=",".join(map(str, Config.snapshot_steps)))
    ...
    print(f"[potts_gen_time_evolution] device={device}")
    ...
    base = f"{cfg.tag}_q{q}_{H}x{W}_steps{max_steps}"
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        f.attrs["grid_size"] = H
        f.attrs["q"] = q
        f.attrs["snapshot_steps"] = json.dumps(snapshot_steps)
        f.attrs["layout"] = "states/final_spins_step{step} is (N, R, H, W) int8"
        ...
        for step in snapshot_steps:
            dset = grp_s.create_dataset(
                f"final_spins_step{step}",
                shape=(N, R, H, W),
                dtype=np.int8,
                chunks=(1, min(R, max(1, repeat_chunk)), H, W),
                compression="gzip",
            )
            step_to_dataset[int(step)] = dset

        outer = range(0, N, P)
        if tqdm:
            outer = tqdm(outer, desc="potts_gen time-evolution param-batches", total=(N + P - 1) // P)
        ...
        for pb in outer:
            ...
            for rb in range(0, R, repeat_chunk):
                ...
                for step in snapshot_steps:
                    ...
                    final_cpu = states[:, 0].detach().cpu().numpy().astype(np.int8, copy=False)
                    final_cpu = final_cpu.reshape(Pb, Rc, H, W)
                    step_to_dataset[step][pb:pe, rb:re, :, :] = final_cpu
    ...
    meta = {
        ...
        "description": {
            ...
            "states/final_spins_step{step}": "(N,R,H,W) int8 raw final spins for each snapshot",
        },
    }
    js_path.write_text(json.dumps(meta, indent=2))

    print(f"[potts_gen_time_evolution] wrote: {h5_path}")
    print(f"[potts_gen_time_evolution] wrote: {js_path}")

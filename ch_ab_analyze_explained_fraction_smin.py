...
def local_forward_smin_J(
    ...
    for i0 in tqdm(range(0, N, int(batch_size)), desc="forward smin_J batches", leave=False):
...
def run_descriptor_block(
    ...
    print(f"[FORWARD_SMIN_J] {warp}_{mode} | {desc_key} | forward map smallest singular value")
    ...
    plot_explained_maps(
        outdir=inv_dir,
        title_base=f"Forward map singular value smin(J) | {warp} | {mode} | Y={desc_key} | q={q}",
        X=X,
        z=smin_J,
        xdim0=xdim0,
        xdim1=xdim1,
    ...
    plot_explained_maps(
        outdir=inv_dir,
        title_base=f"Forward map log10 singular value log10(smin(J)) | {warp} | {mode} | Y={desc_key} | q={q}",
        X=X,
        z=np.log10(smin_J + 1e-30),
        xdim0=xdim0,
        xdim1=xdim1,
        dpi=int(cfg["dpi"]),
        hm_bins0=int(cfg["hm_bins0"]),

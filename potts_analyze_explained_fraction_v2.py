...
class Config:
    ...
    h5: str = "potts_data/20260123_025553Z/corr2d/potts_sims_q3_128x128_corr2d.h5"
...
@torch.no_grad()
def local_forward_smin_J(
    X: torch.Tensor,          # (N,p) standardized if you want
    Y: torch.Tensor,          # (N,qy)
    idxY: torch.Tensor,       # (N,kY)
    ridge_x: float = 1e-8,    # stabilizer on X-block
    batch_size: int = 256,
) -> np.ndarray:
    """
    For each i, on the Y-neighborhood J_i = {i} ∪ N_i^y,
    fit forward ridge: Ys ≈ Xs J, and return sigma_min(J).
    """
    device = X.device
    N, p = X.shape
    kY = idxY.shape[1]
    ...
    I_p = torch.eye(p, device=device, dtype=torch.float32)[None, :, :]
    ...
    for i0 in range(0, N, int(batch_size)):
        ...
        # uniform weights, same centering structure as your current code
        ...
        # Sxx = Xs^T Xs (B,p,p); Sxy = Xs^T Ys (B,p,qy)
        Sxx = torch.bmm(Xs.transpose(1, 2), Xs)
        Sxy = torch.bmm(Xs.transpose(1, 2), Ys)

        trSxx = Sxx.diagonal(dim1=1, dim2=2).sum(dim=1).clamp_min(0.0)
        lam = (float(ridge_x) * trSxx / float(max(p, 1))).to(torch.float32)

        # J = (Sxx + lam I)^{-1} Sxy   (B,p,qy)
        J = torch.linalg.solve(Sxx + lam[:, None, None] * I_p, Sxy)

        # singular values via eigenvalues of JJ^T (B,p,p)
        G = torch.bmm(J, J.transpose(1, 2))
        G = 0.5 * (G + G.transpose(1, 2))  # symmetrize
        ev = torch.linalg.eigvalsh(G).clamp_min(0.0)  # (B,p)
        smin = torch.sqrt(ev[:, 0])  # smallest singular value
...
@torch.no_grad()
def local_lle_anchor_score(
    X: torch.Tensor,      # (N,p)
    Y: torch.Tensor,      # (N,q)
    idxY: torch.Tensor,   # (N,k) neighbors (exclude self)
    eps_lle: float = 1e-8,
    eta: float = 1e-8,
    batch_size: int = 256,
) -> dict:
    """
    LLE barycentric weights from Y-neighborhood geometry, then predict x_i from neighbors.
    Returns:
      lle_unexplained: (N,)
      lle_explained:   (N,)
      num_err:         (N,)  ||x_i - xhat||^2
      denom_scale:     (N,)  mean_j ||x_j - x_i||^2
    """
    device = X.device
    N, p = X.shape
    k = idxY.shape[1]

    I = torch.eye(k, device=device, dtype=torch.float32)[None, :, :]
    ones = torch.ones((1, k, 1), device=device, dtype=torch.float32)

    num = torch.empty((N,), device=device, dtype=torch.float32)
    denom = torch.empty((N,), device=device, dtype=torch.float32)
    u = torch.empty((N,), device=device, dtype=torch.float32)
    e = torch.empty((N,), device=device, dtype=torch.float32)

    for i0 in range(0, N, int(batch_size)):
        i1 = min(N, i0 + int(batch_size))
        B = i1 - i0

        neigh = idxY[i0:i1]                  # (B,k)
        Yn = Y[neigh]                        # (B,k,q)
        y0 = Y[i0:i1][:, None, :]            # (B,1,q)
        Dy = (Yn - y0).to(torch.float32)     # (B,k,q)

        # C = Dy Dy^T  (B,k,k)
        C = torch.bmm(Dy, Dy.transpose(1, 2))
        trC = C.diagonal(dim1=1, dim2=2).sum(dim=1).clamp_min(0.0)
        lam = (float(eps_lle) * trC / float(max(k, 1))).to(torch.float32)
        A = C + lam[:, None, None] * I[:, :, :]

        # weights: (C+lamI) w = 1, then normalize sum(w)=1
        w = torch.linalg.solve(A, ones.expand(B, k, 1)).squeeze(-1)  # (B,k)
        w = w / w.sum(dim=1, keepdim=True).clamp_min(1e-18)

        Xn = X[neigh].to(torch.float32)      # (B,k,p)
        x0 = X[i0:i1].to(torch.float32)      # (B,p)
        xhat = (w[:, :, None] * Xn).sum(dim=1)  # (B,p)

        r = x0 - xhat
        num_b = (r * r).sum(dim=1)  # (B,)

        # scale by neighborhood radius in control space
        dx = Xn - x0[:, None, :]
        denom_b = (dx * dx).sum(dim=2).mean(dim=1)  # mean_j ||x_j - x_i||^2

        u_b = (num_b / (denom_b + float(eta))).clamp(0.0, 10.0)
        e_b = (1.0 - u_b).clamp(0.0, 1.0)
    ...
    return dict(
        lle_unexplained=u.detach().cpu().numpy(),
        lle_explained=e.detach().cpu().numpy(),
        num_err=num.detach().cpu().numpy(),
        denom_scale=denom.detach().cpu().numpy(),
    )
...
def main() -> None:
    ...
    ap.add_argument("--null_perms", type=int, default=16)
    ...
    smin_J = local_forward_smin_J(
        X=Xt, Y=Yt, idxY=idxY_t,
        ridge_x=float(cfg.ridge_x),
        batch_size=int(cfg.batch_size),
    )

    lle = local_lle_anchor_score(
        X=Xt, Y=Yt, idxY=idxY_t,
        eps_lle=1e-3, eta=1e-18,
        batch_size=int(cfg.batch_size),
    )
    ...
    metrics["smin_J"] = smin_J
    metrics["log10_smin_J"] = np.log10(smin_J + 1e-30)

    metrics["lle_explained"] = lle["lle_explained"]

    # --- permutation-calibrated correction ---
    B = int(args.null_perms)
    rng = np.random.default_rng(int(args.null_seed))

    e_true = metrics["explained_frac"].astype(np.float64)
    null_accum = np.zeros_like(e_true, dtype=np.float64)

    for b in range(B):
        perm = rng.permutation(X_use.shape[0])
        Xt_perm = to_t(X_use[perm], device=device)  # same scaling as true X_use
        m_perm = local_explainedcov_metrics_LOO(
            X=Xt_perm,
            Y=Yt,
            idxY=idxY_t,
            dY=dY_t,
        ...
        null_accum += m_perm["explained_frac"].astype(np.float64)

    e_null = null_accum / max(B, 1)

    eps = 1e-6
    e_star = (e_true - e_null) / (1.0 - e_null + eps)
    e_star = np.clip(e_star, 0.0, 1.0).astype(np.float32)

    # store
    metrics["explained_frac_nullmean"] = e_null.astype(np.float32)
    metrics["explained_frac_corrected"] = e_star
    ...
    if cfg.save_scatter:
        scatter_tf(temp, frac, metrics["lle_explained"],
                   f"{descriptor}: local LLE explained fraction",
                   figs_dir / "scatter_lle_explained", dpi=cfg.dpi, vmin=0.0, vmax=1.0)
        scatter_tf(temp, frac, metrics["smin_J"],
                   f"{descriptor}: smallest singular value of local forward map J",
                   figs_dir / "scatter_smin_J", dpi=cfg.dpi)
        scatter_tf(temp, frac, metrics["log10_smin_J"],
                   f"{descriptor}: log10 smallest singular value of local forward map J",
                   figs_dir / "scatter_log10_smin_J", dpi=cfg.dpi)
        scatter_tf(temp, frac, metrics["explained_frac_corrected"],
                   f"{descriptor}: null-calibrated explained fraction",
                   figs_dir / "scatter_explained_frac_corrected", dpi=cfg.dpi, vmin=0.0, vmax=1.0)
    ...
    if cfg.save_heatmaps:
        heatmap_binned_tf(temp, frac, metrics["lle_explained"],
                          f"{descriptor}: heatmap local LLE explained fraction",
                          figs_dir / "heatmap_lle_explained", 
                          bins_t=cfg.hm_bins_temp, bins_f=cfg.hm_bins_frac,
                          sigma_px=cfg.hm_sigma_px, clip=cfg.hm_clip, dpi=cfg.dpi)
        heatmap_binned_tf(temp, frac, metrics["smin_J"],
                          f"{descriptor}: heatmap smallest singular value of local forward map J",
                          figs_dir / "heatmap_smin_J",
                          bins_t=cfg.hm_bins_temp, bins_f=cfg.hm_bins_frac,
                          sigma_px=cfg.hm_sigma_px, clip=cfg.hm_clip, dpi=cfg.dpi)
        heatmap_binned_tf(temp, frac, metrics["log10_smin_J"],
                          f"{descriptor}: heatmap log10 smallest singular value of local forward map J",
                          figs_dir / "heatmap_log10_smin_J",
                          bins_t=cfg.hm_bins_temp, bins_f=cfg.hm_bins_frac,
                          sigma_px=cfg.hm_sigma_px, clip=cfg.hm_clip, dpi=cfg.dpi)
        heatmap_binned_tf(temp, frac, metrics["explained_frac_corrected"],
                          f"{descriptor}: heatmap null-calibrated explained fraction",
                          figs_dir / "heatmap_explained_frac_corrected",
                          bins_t=cfg.hm_bins_temp, bins_f=cfg.hm_bins_frac,
                          sigma_px=cfg.hm_sigma_px, clip=cfg.hm_clip, dpi=cfg.dpi)
